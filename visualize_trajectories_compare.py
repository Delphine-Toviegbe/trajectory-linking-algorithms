...
# Local import
from link_trajectories import build_chains, parse_iso

from compare_algorithms import (
    build_cost_matrix,
    algo_greedy,
    algo_hungarian,
    algo_bipartite_ghost
)
...
def main():
    ...
    parser.add_argument("--algo", default="greedy",
                    choices=["greedy", "hungarian", "bipartite"])
    ...
    C = build_cost_matrix(trajectories, args.max_gap, args.max_dist)

    if args.algo == "greedy":
        links = algo_greedy(trajectories, C)

    elif args.algo == "hungarian":
        links = algo_hungarian(trajectories, C)

    elif args.algo == "bipartite":
        links = algo_bipartite_ghost(trajectories, C)
    
    chains = build_chains(trajectories, links)
    
    if args.output is None:
        args.output = f"trajectories_linked_{args.algo}.mp4"
